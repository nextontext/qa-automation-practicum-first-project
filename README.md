...
Уникальный SHA-1 (или SHA-256 в новых конфигурациях) идентификатор коммита.

Обычно используется:
- как ссылка на конкретную версию кода
- для сравнения изменений
- для отката к состоянию

## Лог (история коммитов)
Буквально для просмотра истории изменений (когда менялся файл, понять историю фичи)

## HEAD
Указатель на текущий коммит (текущее положение репозитория)
...
| modified | изменён |
| staged | добавлен в индекс |
| committed | сохранён в истории |
